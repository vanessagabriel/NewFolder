...
public class AdmUsuariosTest {
	...
	@Test
	public void siAsignoRolUsuario() throws PeruBankException {
		// Ejecutar
		AdmUsuarios adm = new AdmUsuarios();
		adm.registrarUsuarios(codigoUsuario, ctaUsuario, password, codigoRol);
		...
		//Asignar el rol
		admRolUsuario.asignarRolUsuarios(usuario.getCodigoUsuario(), codigoRol, estado);

		RolUsuario rolUsuario = admRolUsuario.buscarRolPoUsuario(usuario.getCodigoUsuario());
	...
	@Test
	public void siIgresoDeberiaValidarUsuario(){
		// Cargar los datos de un usuario
		Usuario usuarioIngreso = new Usuario(codigoUsuario, ctaUsuario, password);//, codigoRol
		
		Usuario usuariodevuelto = (Usuario)m.get(usuarioIngreso.getCtaUsuario());
		
		//Validar que el Objeto usuario sea distinto de null
		assertNotNull(usuariodevuelto);
		
		//validar usuario y pasword ingresado
		assertEquals(usuarioIngreso.getCtaUsuario(), usuariodevuelto.getCtaUsuario());
		assertEquals(usuarioIngreso.getPassword(), usuariodevuelto.getPassword());
		
		//extrae rol del usuario
		RolUsuario rolUsuarioDevuelto = (RolUsuario)m.get(usuariodevuelto.getCodigoUsuario());

		//Validar que el Objeto rolUsuarioDevuelto sea distinto de null
		assertNotNull(rolUsuarioDevuelto);
		
		//extraer opciones del rol
		OpcionRol opcionRolDevuelto = (OpcionRol)m.get(rolUsuarioDevuelto.getCodigoRol());
